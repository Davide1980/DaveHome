...
class IdealistaScraper(BaseScraper):
    """Scraper for Idealista.it website."""

    def get_site_name(self) -> str:
        return "Idealista.it"

    def build_search_url(self) -> str:
        """Build Idealista.it search URL."""
        criteria = self.search_criteria

        # Idealista.it URL structure
        base_url = "https://www.idealista.it/vendita-case/monselice-padova/"
        ...
        if criteria.get('rooms'):
            params.append(f"minCamere={criteria['rooms']}")
    ...
    def parse_listing(self, listing_element) -> Dict:
        """Parse a single listing from Idealista.it."""
        try:
            listing = {
            ...
            # Title and URL
            title_elem = listing_element.find('a', class_='item-link')
            if title_elem:
                listing['title'] = title_elem.get_text(strip=True)
                listing['url'] = "https://www.idealista.it" + title_elem.get('href', '')

            # Price
            price_elem = listing_element.find('span', class_='item-price')
            if price_elem:
            ...
            # Area
            area_elem = listing_element.find('span', class_='item-detail', string=re.compile(r'm²'))
            if area_elem:
            ...
            # Rooms
            rooms_elem = listing_element.find('span', class_='item-detail', string=re.compile(r'camere?|local', re.IGNORECASE))
            if rooms_elem:
            ...
            # Alternative: parse from detail string
            detail_elem = listing_element.find('span', class_='item-detail-char')
            if detail_elem:
                ...
                if not listing['rooms']:
                    rooms_match = re.search(r'(\d+)\s*camer', detail_text, re.IGNORECASE)
                    if rooms_match:
                        listing['rooms'] = int(rooms_match.group(1))

            # Location
            location_elem = listing_element.find('span', class_='item-location')
            if location_elem:
                listing['location'] = location_elem.get_text(strip=True)

            # Description
            desc_elem = listing_element.find('div', class_='item-description')
            if desc_elem:
    ...
    def scrape(self) -> List[Dict]:
        """Scrape Idealista.it for listings."""
        print(f"\n{'='*60}")
        ...
        if not listing_elements:
            listing_elements = soup.find_all('div', class_='item-info-container')
